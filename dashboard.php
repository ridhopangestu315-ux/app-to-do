...
====================================================
File ini adalah halaman dashboard yang memerlukan login.

Alur:
1. File ini dimulai dengan session_start()
2. Cek apakah user sudah login dengan cek $_SESSION['user_id']
3. Jika belum login, redirect ke halaman login
4. Jika sudah login, tampilkan dashboard
5. Gunakan $_SESSION['nama'] untuk menampilkan nama user
6. Semua API call di script.js sudah terhubung ke backend

CATATAN:
- File ini adalah duplikat dari index.html dengan tambahan PHP session check
- Semua fitur dan desain tetap sama seperti sebelumnya
- Hanya ditambahkan logout button di header
====================================================
*/
...
// User sudah login, ambil data dari session
...
<body>
  ...
  <div class="wadah-aplikasi">
    <aside class="menu-samping">
      ...
      <div style="margin-top: 24px; padding-top: 24px; border-top: 1px solid var(--line);">
        <button id="tombolLogout" type="button" style="width: 100%; padding: 12px 16px; border: 1px solid var(--red); border-radius: 12px; background: transparent; color: var(--red); font-size: 14px; font-weight: 700; cursor: pointer; transition: 0.2s ease;" onmouseover="this.style.background = 'rgba(225, 29, 72, 0.1)'" onmouseout="this.style.background = 'transparent'">
          Logout
        </button>
      </div>
    </aside>
    ...
    <main class="konten-utama">
      <header class="kepala-halaman">
        <div class="kepala-copy">
          ...
          <h1 id="teksSapaan">Selamat datang kembali, <?php echo htmlspecialchars($nama_user); ?> 👋</h1>
        </div>
        <div class="header-actions">
          <div class="tanggal-header">
            <span id="teksTanggalRealtime">Rabu, 20 Mei 2026</span>
            <small id="teksJamRealtime">00.00</small>
          </div>
          <div id="fotoProfilHeader" class="foto-profil-header" aria-label="Profil pengguna">
      ...
      <section id="dashboard" class="halaman halaman-aktif">
        <section class="hero-dashboard">
          <div class="hero-copy">
            ...
            <h2 id="teksSapaanHero">Selamat datang kembali, <?php echo htmlspecialchars($nama_user); ?> 👋</h2>
  ...
  <!-- SCRIPT MODIFIKASI UNTUK DATABASE -->
  <script>
    // ====================================================
    // MODIFIKASI UNTUK LOGOUT BUTTON
    // ====================================================
    document.getElementById('tombolLogout').addEventListener('click', async function() {
